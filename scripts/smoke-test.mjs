...
const adminJar = { ...jar };
{
  ...
  console.log("sharing a profile");
  {
    ...
    await asAdmin(async () => { for (const n of (await call("GET", "/api/notifications?limit=50")).data.items.filter((x) => /^profile_/.test(x.type) || (x.actor_id === userId && /shared editor/.test(x.title)))) await call("DELETE", `/api/notifications/${n.id}`); });
  }
