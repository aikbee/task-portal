...
console.log("chat (friends by code, one-to-one messages)");
{
  ...
  const memberPic = await as(adminJar, () => call("PUT", `/api/chat/conversations/${gid}/avatar`, { body: { avatar: "preset:rocket" } }));
  check("a member cannot change the group picture -> 403", memberPic.status === 403);
  ...
  check("history is still readable (with the two retention system lines)", keepsHistory.status === 200 && keepsHistory.data.length === 23, String(keepsHistory.data?.length));
