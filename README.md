...
| **Moderation** (admin) | Reported chat messages with a snapshot of each (delete the message for everyone or dismiss), every conversation's kind, members, message count, file size, retention and open reports (never the text), per-chat retention, full exports, deleting a conversation, an instance-wide cap on how long chat history is kept, and the GIPHY / Tenor key that turns on GIF search in chat. The sidebar badge counts open reports. |
...
| PUT | `/api/chat/admin/settings` | admin: `{ max_retention_days?, gif_provider?, gif_api_key? }` — history cap (purges at once) and the GIF search key (stored server-side, reported only as `gif_key_set`) |
