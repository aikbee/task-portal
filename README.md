...
| **Mobile app** (admin) | Publish a build of the native Android app (the signed APK from `dist/`): the file itself says its package, version and signing key, so an older build or one signed with another key is refused. Phones with the app check for the newest build, download it and update themselves. |
