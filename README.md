...
| **Backgrounds** (admin) | Controls which of the 38 animated background styles users may pick (twenty-four are WebGL scenes rendered with three.js, loaded only when active) in Preferences, the default for new browsers, and an optional lock that forces the default on everyone. Live previews of every style. Stored instance-wide in `app_settings`. The same backgrounds are served on their own at `/bg?style=&theme=&accent=&intensity=&animate=` (public, no data, only the animation; `window.TaskPortalBg.set({...})` changes it live) so the native Android app can show them behind its own screens. |
