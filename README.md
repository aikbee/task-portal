...
| Role | Access |
| --- | --- |
| **admin** | Everything, plus the **Users** module: create accounts, set roles, disable, reset passwords, see each account's sign-in methods (passkeys, Google, 2FA) and remove a lost passkey, unlink Google or reset two-factor authentication (the account keeps its password and is notified), delete (the last active admin and your own account are protected) |
