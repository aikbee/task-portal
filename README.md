...
| **Mobile app** (admin) | Publish a build of the native Android app (the signed APK from `dist/`): the file itself says its package, version and signing key, so an older build or one signed with another key is refused. Publishing announces the build on the live stream, so an app that is open (or connected in the background) hears about it at once; the others check when they come to the front and every few hours. In front the app shows an update dialog; in the background it downloads and installs by itself where Android allows it, otherwise a notification leads to the dialog. |
...
| **Bulk edit** | Tick tasks in the Tasks table and press **Edit** on the selection bar: set status, priority or project, add / replace / remove assignees, set, clear or move dates by a number of days, add or remove tags. Only what is set changes. Every task still goes through the normal checks, history and webhooks; people get one notification instead of one per task; tasks that cannot take the change are reported and the rest go through; **Undo** in the toast puts the previous values back. Editors and up. |
| **Time** | Hours logged in the profile for a period (this week, month, quarter, year, or any dates), grouped by person, project, task or day, with the share of the total, the count of entries and, for tasks and projects, logged against the estimate. Clicking a row drills down (a person or project to their tasks, a day to that day, a task to its page); a per-day bar chart; the entries themselves in a searchable table; CSV export of the totals and of the entries. **Report** opens the same period and filters as a print-ready timesheet (`/report/time?from=&to=&group=&project_id=&user_id=`): a summary, totals per person, project, task or day with the grand total, the entries under each group with subtotals, optional notes, decimal hours and signature lines; **Save as PDF** uses the browser's print dialog. |
| **Backgrounds** (admin) | Controls which of the 39 animated background styles users may pick (twenty-five are WebGL scenes rendered with three.js, loaded only when active) in Preferences, the default for new browsers, and an optional lock that forces the default on everyone. Live previews of every style. Stored instance-wide in `app_settings`. The same backgrounds are served on their own at `/bg?style=&theme=&accent=&intensity=&animate=` (public, no data, only the animation; `window.TaskPortalBg.set({...})` changes it live) so the native Android app can show them behind its own screens. |
...
| **Background** | Thirty-nine animated styles (CSS scenes such as *Aurora*, *Mesh*, *Orbs*, *Stars*, *Rain*, *Snow* and WebGL scenes such as *Galaxy*, *Earth*, *Ocean*, *Balloons*, *Hearts*, *Jellyfish*, *Ghosts*, *Portal*, *Wisps*, *Ringed planet*, *Nebula*, *Solar system*, *Grassland* with wind-swayed grass and grazing cows, *City drive* seen from the driver's seat: a bonnet in your accent colour, a steering wheel that follows the bends, buildings, lamps, trees and traffic sliding past at about 50 km/h, sunlit by day and neon-lit at night, *Neural network*: hundreds of glowing nodes drifting in depth, links that fade in and out as they pass, signals that hop from node to node and make them fire, and a soft glow around your mouse pointer, *Frozen peaks*: low-poly ice mountains under falling snow with big out-of-focus flakes up close, mist drifting through the valley, a glowing frozen lake, a lone climber in your accent colour on a rock pinnacle, and at night a moon, aurora and the odd shooting star; by day the same range in sunlight, *Lucky cat*: a plump beckoning cat with an accent-coloured collar and bow, a 發 plaque and gold at its feet, in front of a golden sunburst with red lanterns, while coins, notes and ingots rain down and sparkles twinkle; cream and sunlit by day, deep red and lamplit at night, *Campsite*: a forest clearing with a crackling campfire under a cooking pot on a tripod, sparks and smoke rising, a tent in your accent colour glowing from inside, log benches and a lantern on a stump; at night stars, the Milky Way, fireflies, an owl blinking in the trees and the odd shooting star, by day a low sun, clouds, birds and falling autumn leaves, *Koi pond*: a lotus pond seen from above with eight kinds of koi gliding, bending as they turn and casting soft shadows on the pebbles, light dancing on the pond floor, real ripple rings when a koi touches the surface, a petal lands or an insect alights, the view below bending with the rings, lily pads and lotus flowers in your accent colour, and koi that gather round the mouse pointer; floating lanterns, a moon reflection and fireflies at night, *Ink landscape*: a living Chinese ink painting on rice paper, four ridges painted with a dry-brush rim, washes and texture strokes that dissolve into mist, drifting mist, a fishing boat crossing the river, red-crowned cranes, a pine leaning off a cliff, a vermilion sun, and a line of Wang Wei's poem with a seal in your accent colour; by night moonlit ink, the moon on the water, a lantern on the boat and the poem's moonlit couplet) or none; subtle / normal / vivid intensity, shuffle, pausable. **Show background only** (`⌘⇧.`, the top-bar wallpaper button, account menu, Preferences → Background, or the lock screen) fades every layer away so just the background plays; the first click or key press animates everything back. |
...
| GET | `/api/app/releases` | admin: every published build of the Android app |
| POST | `/api/app/releases` | admin: publish a build — multipart `file` (the APK) + `notes`; refused when it is not our app, not newer, unsigned or signed with another key |
| DELETE | `/api/app/releases/:id` | admin: remove a build (phones keep what they have; the newest remaining one is offered) |
| GET | `/api/app/android/latest?package=` | public: the newest build for that package — `{ release: { version_code, version_name, size_bytes, sha256, notes, url } \| null }`; the app asks on start, when it comes to the front and daily |
| GET | `/api/app/android/download/:id` | public: the APK, with `x-checksum-sha256` |
