...
| GET/POST | `/api/chat/conversations/:id/messages` | `?before=&limit=` pages backwards / JSON `{ body }` or multipart `body` + `files[]` (photos, or one voice note with `duration` in ms); messages carry `attachments[]` (`kind` image/audio) |
