...
| **Backgrounds** (admin) | Controls which of the 31 animated background styles users may pick (seventeen are WebGL scenes rendered with three.js, loaded only when active) in Preferences, the default for new browsers, and an optional lock that forces the default on everyone. Live previews of every style. Stored instance-wide in `app_settings`. |
...
| **Background** | Thirty-one animated styles (CSS scenes such as *Aurora*, *Mesh*, *Orbs*, *Stars*, *Rain*, *Snow* and WebGL scenes such as *Galaxy*, *Earth*, *Ocean*, *Balloons*, *Hearts*, *Jellyfish*, *Ghosts*, *Portal*, *Wisps*, *Ringed planet*, *Nebula*, *Solar system*) or none; subtle / normal / vivid intensity, shuffle, pausable. **Show background only** (`⌘⇧.`, the top-bar wallpaper button, account menu, Preferences → Background, or the lock screen) fades every layer away so just the background plays; the first click or key press animates everything back. |
...
- **Read receipts.** Every message you send carries a tick: ✓ sent, ✓✓ read by some, bright ✓✓ read by everyone in the chat. In groups, pressing the ticks lists who has read the message and who has not yet. Opening a chat with unread messages lands on an "Unread messages" line rather than the bottom. Receipts come from each member's read position, so nothing extra is stored.
...
- **Live updates** come over Server-Sent Events (`/api/chat/stream`, events `message`, `message_updated`, `read`, `friends`, `conversation`, `typing`, `reaction`); if the stream cannot connect the page polls every 5 seconds. New messages and friend requests also raise notifications (and pushes) in the **Chat** category, one bell entry per conversation.
...
| GET/POST | `/api/chat/conversations/:id/messages` | `?before=&limit=` pages backwards, `?around=<id>` loads a window round one message / JSON `{ body }` or multipart `body` + `files[]` (photos, or one voice note with `duration` in ms); messages carry `attachments[]` (`kind` image/audio) |
| GET | `/api/chat/photos/:id` | a photo or voice note, for members of its conversation (`?download=1`; byte ranges supported) |
...
| PUT/DELETE | `/api/chat/messages/:id` | `{ body }` edits your own message (sets `edited_at`) / soft-deletes yours or, as group owner, anyone's (`deleted_at`, content and files removed) |
