...
| **Backgrounds** (admin) | Controls which of the 22 animated background styles users may pick (eight are WebGL scenes rendered with three.js, loaded only when active) in Preferences, the default for new browsers, and an optional lock that forces the default on everyone. Live previews of every style. Stored instance-wide in `app_settings`. |
...
| **Background** | Twenty-two animated styles (CSS scenes such as *Aurora*, *Mesh*, *Orbs*, *Stars*, *Rain*, *Snow* and WebGL scenes such as *Galaxy*, *Earth*, *Ocean*) or none; subtle / normal / vivid intensity, shuffle, pausable. **Show background only** (top-bar wallpaper button, account menu, Preferences → Background, or the lock screen) fades every layer away so just the background plays; the first click or key press animates everything back. |
