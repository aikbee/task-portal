...
| GET/POST | `/api/chat/conversations/:id/messages` | `?before=&limit=` pages backwards, `?around=<id>` loads a window round one message / JSON `{ body, reply_to? }` or multipart `body` (+ `reply_to`) + `files[]` (photos and other files) or `voice` + `duration` (ms); messages carry `attachments[]` (`kind` image/audio/file), `reply_to` (quote) and `forwarded` |
