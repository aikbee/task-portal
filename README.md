...
| **Backgrounds** (admin) | Controls which of the 33 animated background styles users may pick (nineteen are WebGL scenes rendered with three.js, loaded only when active) in Preferences, the default for new browsers, and an optional lock that forces the default on everyone. Live previews of every style. Stored instance-wide in `app_settings`. |
