...
| **Backgrounds** (admin) | Controls which of the 14 animated background styles users may pick in Preferences, the default for new browsers, and an optional lock that forces the default on everyone. Live previews of every style. Stored instance-wide in `app_settings`. |
