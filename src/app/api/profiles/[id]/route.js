...
export const PUT = handler(async (request, params, user) => {
  ...
  await find(id, user.home_id);
  const body = await readJson(request);
  const data = pick(body, ["name", "description", "color"]);
  if (data.name === null) throw new HttpError("Name is required.", 400);
  if (data.name) {
    const dup = await queryOne("SELECT id FROM profiles WHERE user_id = ? AND name = ? AND id <> ?", [user.home_id, data.name, id]);
  ...
  await withTransaction(async (conn) => {
    ...
    if (body.is_default) {
      await conn.execute("UPDATE profiles SET is_default = 0 WHERE user_id = ?", [user.home_id]);
      await conn.execute("UPDATE profiles SET is_default = 1 WHERE id = ?", [id]);
    }
  });
  return ok(await find(id, user.home_id));
});
