...
export const POST = handler(async (request, params, user) => {
  ...
  if (upTo > 0) {
    ...
    await execute("UPDATE conversation_members SET last_read_message_id = GREATEST(COALESCE(last_read_message_id, 0), ?), delivered_message_id = GREATEST(COALESCE(delivered_message_id, 0), ?) WHERE conversation_id = ? AND user_id = ?", [upTo, upTo, id, user.id]);
