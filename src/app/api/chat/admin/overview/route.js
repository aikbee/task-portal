  ...
  async () => {
    ...
    const totals = await queryOne(
      `SELECT
        ...
        (SELECT COUNT(*) FROM messages WHERE kind = 'text' AND deleted_at IS NULL) AS messages,
    ...
    const conversations = await query(
      `SELECT c.id, c.kind, c.title, c.avatar, c.avatar_color, c.retention_days, (c.invite_code IS NOT NULL) AS has_invite, c.created_at,
        ...
        (SELECT COUNT(*) FROM messages x WHERE x.conversation_id = c.id AND x.kind = 'text' AND x.deleted_at IS NULL) AS message_count,
