...
function spec(module, tr) {
  switch (module) {
    ...
    case "tasks":
      return { statusMap: TASK_STATUS, statusKey: "status", groupKey: "project_name", columns: [{ key: "title", label: tr("Task") }, { key: "project_name", label: tr("Project") }, { key: "assignee_names", label: tr("Assignees") }, { key: "status", label: tr("Status"), render: (r) => label(TASK_STATUS, r.status) }, { key: "priority", label: tr("Priority"), render: (r) => label(TASK_PRIORITY, r.priority) }, { key: "due_date", label: tr("Due"), render: (r) => (r.due_date ? formatDate(r.due_date) : "—") }] };
    case "info":
