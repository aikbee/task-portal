...
export default function Modal({ open, onClose, title, description, children, footer, size = "md", className }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6" role="dialog" aria-modal="true">
      ...
      >
        {(title || description) && (
          <div className="ui-modal-head flex shrink-0 items-start justify-between gap-4 border-b border-line px-6 py-4">
            <div>
        ...
        <div className="ui-modal-body min-h-0 flex-1 overflow-y-auto px-6 py-5">{children}</div>
        {footer ? <div className="ui-modal-foot flex shrink-0 items-center justify-end gap-2 border-t border-line px-6 py-4">{footer}</div> : null}
      </div>
    </div>,
    document.body
  );
