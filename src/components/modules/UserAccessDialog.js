...
export default function UserAccessDialog({ user, users = [], open, onClose, onSaved }) {
  const tr = useT();
  const toast = useToast();
  const { data, loading } = useFetch(`/api/users/${user?.id}/access`, { enabled: Boolean(open && user) });
  ...
  const copyFrom = async (id) => {
    if (!id) return;
    try {
      const other = await api.get(`/api/users/${id}/access`);
  ...
  const save = async () => {
    setBusy(true);
    try {
      const res = await api.put(`/api/users/${user.id}/access`, { modules_off: [...draft.modules_off], features_off: [...draft.features_off] });
      const n = res.effective.modules_off.length + res.effective.features_off.length;
      toast.success(tr("Access saved"), n ? tr("{n} things are off for {name}.", { n, name: res.user.name }) : tr("Everything is on for {name}.", { name: res.user.name }));
      onSaved?.();
      onClose();
    } catch (e) {
  ...
  return (
    <Modal open={open} onClose={close} size="lg" className="user-access" title={tr("What {name} can use", { name: user?.name ?? "" })} description={tr("What is off disappears from their menus and pages, and the server refuses it too. It applies in every profile they open.")}
